...
Description: SCENARIO 2b: GNLR EMEA STRUCTURED RATES - All measures missing
Test:
'''
import qzsix
import qztable
...
def dataSourceFactory(cfg, key, dataSources, jobTimeStamp, name):
    """
    SCENARIO 2b: GNLR EMEA STRUCTURED RATES - All measures missing
    Simulate missing data for GLOBAL NON-LINEAR-EMEA STRUCTURED RATES VTD
    """
    logger.info(f"SCENARIO 2b: Processing {name} with source {key}")
...
def fetchFromRRA(cfg, key, dataSources, jobTimeStamp, name):
    ...
    # SCENARIO 2b: Force GNLR EMEA STRUCTURED RATES to have no data
    if name == "GLOBAL NON-LINEAR-EMEA STRUCTURED RATES":
        logger.info("=== SCENARIO 2b: Simulating GNLR EMEA STRUCTURED RATES - ALL MEASURES MISSING ===")
