...
Description: SCENARIO 3c: APAC LINEAR RATES - Sov Spread Delta missing, IR01 present
Test:
'''
import qzsix
import qztable
...
def dataSourceFactory(cfg, key, dataSources, jobTimeStamp, name):
    """
    SCENARIO 3c: APAC LINEAR RATES - One measure missing (Sov Spread Delta)
    Simulate Sov Spread Delta missing but IR01 present for APAC LINEAR RATES VTD
    """
    logger.info(f"SCENARIO 3c: Processing {name} with source {key}")
...
def fetchFromRRA(cfg, key, dataSources, jobTimeStamp, name):
    ...
    for measure in fieldsDict.get('measure_names',[]):
        # SCENARIO 3c: For APAC LINEAR RATES, skip Sov Spread Delta to simulate missing data
        if name == "APAC LINEAR RATES" and measure == "Sov Spread Delta":
            logger.info("=== SCENARIO 3c: Simulating APAC LINEAR RATES - Sov Spread Delta MISSING ===")
