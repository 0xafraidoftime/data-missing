...
Description: SCENARIO 14: Partial Source Recovery - One source recovers, other remains down
Test:
'''
import qzsix
import qztable
...
def dataSourceFactory(cfg, key, dataSources, jobTimeStamp, name):
    """
    SCENARIO 14: Partial Source Recovery
    - First snap: Both CIRT and Legacy sources down for GNLR AMRS
    - Second snap: CIRT source recovers, Legacy source remains down
    Tests mixed source availability and partial recovery
    """
    logger.info(f"SCENARIO 14: Processing {name} with source {key}")
    
    # Determine snap timing
    current_hour = jobTimeStamp.hour
    is_first_snap = current_hour < 12
...
def fetchFromRRA(cfg, key, dataSources, jobTimeStamp, name, is_first_snap):
    ...
    # SCENARIO 14: Handle GNLR AMRS CIRT source based on snap timing
    if name == "GLOBAL NON-LINEAR-AMRS STRUCTURED RATES":
        if is_first_snap:
            logger.info("=== SCENARIO 14: First Snap - GNLR AMRS CIRT SOURCE DOWN ===")
            # Return empty tables for all measures in first snap
            ...
            logger.info("=== SCENARIO 14: Second Snap - GNLR AMRS CIRT SOURCE RECOVERED ===")
            # CIRT source recovers in second snap - normal processing
    
    # Normal processing for other VTDs or recovered CIRT source
    for measure in fieldsDict.get('measure_names',[]):
        querySet = {}
        ...
        # Original data clearing conditions (modified to allow GNLR AMRS recovery testing)
        ...
        # Allow recovery testing for GNLR AMRS
...
def fetchFromLegacy(cfg, key, dataSources, jobTimeStamp, name, is_first_snap):
    ...
    # SCENARIO 14: Legacy source remains down in both snaps for GNLR AMRS
    if name == "GLOBAL NON-LINEAR-AMRS STRUCTURED RATES":
        if is_first_snap:
            logger.info("=== SCENARIO 14: First Snap - GNLR AMRS LEGACY SOURCE DOWN ===")
        else:
            logger.info("=== SCENARIO 14: Second Snap - GNLR AMRS LEGACY SOURCE STILL DOWN ===")
        
        # Return empty tables for all legacy measures in both snaps
        for measure in fieldsDict.get('measure_names',[]):
            ...
            # Map legacy measure names for error reporting
