...
Description: SCENARIO 7: VTD Recovery - First snap VTD missing, Second snap VTD available
Test:
'''
import qzsix
import qztable
...
def dataSourceFactory(cfg, key, dataSources, jobTimeStamp, name):
    """
    SCENARIO 7: VTD Recovery Testing
    - First snap (before 9:30): AMRS LINEAR RATES missing all data
    - Second snap (COB): AMRS LINEAR RATES data available
    This tests the system's ability to handle VTD recovery scenarios
    """
    logger.info(f"SCENARIO 7: Processing {name} with source {key} at {jobTimeStamp}")
...
def fetchFromRRA(cfg, key, dataSources, jobTimeStamp, name, is_first_snap):
    ...
    # SCENARIO 7: Handle AMRS LINEAR RATES based on snap timing
    if name == "AMRS LINEAR RATES":
        if is_first_snap:
            logger.info("=== SCENARIO 7: First Snap - AMRS LINEAR RATES MISSING ===")
            ...
            logger.info("=== SCENARIO 7: Second Snap - AMRS LINEAR RATES RECOVERED ===")
            # Normal processing for second snap - data is now available
    
    # Normal processing for other VTDs or second snap
    for measure in fieldsDict.get('measure_names',[]):
        querySet = {}
...
def fetchFromLegacy(cfg, key, dataSources, jobTimeStamp, name, is_first_snap):
    ...
    # Normal processing for legacy sources (AMRS uses cirt_unified_screen, not legacy)
    for measure in fieldsDict.get('measure_names',[]):
        querySet = {}
        querySet.update({'Measure':measure})
        querySet.update(fieldsDict)
