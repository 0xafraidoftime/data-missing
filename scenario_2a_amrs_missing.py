...
Description: SCENARIO 2a: AMRS LINEAR RATES - All measures missing
Test:
'''
import qzsix
import qztable
...
def dataSourceFactory(cfg, key, dataSources, jobTimeStamp, name):
    """
    SCENARIO 2a: AMRS LINEAR RATES - All measures missing
    Simulate missing data for AMRS LINEAR RATES VTD
    """
    logger.info(f"SCENARIO 2a: Processing {name} with source {key}")
...
def fetchFromRRA(cfg, key, dataSources, jobTimeStamp, name):
    ...
    # SCENARIO 2a: Force AMRS LINEAR RATES to have no data
    if name == "AMRS LINEAR RATES":
        logger.info("=== SCENARIO 2a: Simulating AMRS LINEAR RATES - ALL MEASURES MISSING ===")
...
def fetchFromLegacy(cfg, key, dataSources, jobTimeStamp, name):
    ...
    # SCENARIO 2a: AMRS uses cirt_unified_screen, not legacy, so normal processing
    for measure in fieldsDict.get('measure_names',[]):
        querySet = {}
        querySet.update({'Measure':measure})
        querySet.update(fieldsDict)
