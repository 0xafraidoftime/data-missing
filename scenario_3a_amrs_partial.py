...
Description: SCENARIO 3a: AMRS LINEAR RATES - Vega missing, other measures present
Test:
'''
import qzsix
import qztable
...
def dataSourceFactory(cfg, key, dataSources, jobTimeStamp, name):
    """
    SCENARIO 3a: AMRS LINEAR RATES - One measure missing (Vega)
    Simulate Vega missing but other measures present for AMRS LINEAR RATES VTD
    """
    logger.info(f"SCENARIO 3a: Processing {name} with source {key}")
...
def fetchFromRRA(cfg, key, dataSources, jobTimeStamp, name):
    ...
    for measure in fieldsDict.get('measure_names',[]):
        # SCENARIO 3a: For AMRS LINEAR RATES, skip Vega to simulate missing data
        if name == "AMRS LINEAR RATES" and measure == "Vega":
            logger.info("=== SCENARIO 3a: Simulating AMRS LINEAR RATES - Vega MISSING ===")
