...
Description: SCENARIO 3b: GNLR EMEA STRUCTURED RATES - Vega missing, IR01 present
Test:
'''
import qzsix
import qztable
...
def dataSourceFactory(cfg, key, dataSources, jobTimeStamp, name):
    """
    SCENARIO 3b: GNLR EMEA STRUCTURED RATES - One measure missing (Vega)
    Simulate Vega missing but IR01 present for GLOBAL NON-LINEAR-EMEA STRUCTURED RATES VTD
    """
    logger.info(f"SCENARIO 3b: Processing {name} with source {key}")
...
def fetchFromRRA(cfg, key, dataSources, jobTimeStamp, name):
    ...
    for measure in fieldsDict.get('measure_names',[]):
        # SCENARIO 3b: For GNLR EMEA STRUCTURED RATES, skip Vega to simulate missing data
        if name == "GLOBAL NON-LINEAR-EMEA STRUCTURED RATES" and measure == "Vega":
            logger.info("=== SCENARIO 3b: Simulating GNLR EMEA STRUCTURED RATES - Vega MISSING ===")
        ...
        if measure in ["IR Delta", "Inflation Delta"] and name == "GLOBAL NON-LINEAR-AMRS STRUCTURED RATES":
            rraExposureTable = qztable.Table(rraExposureTable.getSchema())
            rraExpTable = qztable.Table(rraExpTable.getSchema())
        # Note: Modified condition to allow IR01 data for GNLR EMEA in this scenario
